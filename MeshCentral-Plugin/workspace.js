...
module.exports.workspace = function workspacePlugin(parent) {
    ...
    obj.server_startup = function () { console.log('[MeshCentral-Workspace] Plugin 0.4.2 loaded'); };
    ...
    obj.onWebUIStartupEnd = function () {
        ...
        var endpoint = function (asset) { var url = new URL('pluginadmin.ashx', window.location.href); url.searchParams.set('pin', 'workspace'); url.searchParams.set('asset', asset); url.searchParams.set('v', '0.4.2'); return url.href; };
        ...
        if (!document.getElementById('workspace-plugin-css')) { var style = document.createElement('link'); style.id = 'workspace-plugin-css'; style.rel = 'stylesheet'; style.href = endpoint('main.css'); (document.head || document.documentElement).appendChild(style); }
    ...
    obj.handleAdminReq = function (req, res, user) {
        ...
        if (asset === 'status') { const session = pluginModule.status(user, req && req.query && req.query.id); if (!session) { sendJson(res, 404, { ok: false, error: 'Session not found.' }); return; } sendJson(res, 200, { ok: true, result: session }); return; }
    ...
    obj.handleAdminPostReq = function (req, res, user) {
        ...
        if (asset === 'start') { handlePromise(res, pluginModule.start(user, body.nodeId)); return; }
