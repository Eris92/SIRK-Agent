...
{
    InspectJson("Agent Core / Security State", Path.Combine(agentDirectory, "security-state.json"), MissingBehavior.Critical),
    InspectJson("Policy / Heartbeat", Path.Combine(agentDirectory, "heartbeat-latest.json"), MissingBehavior.Critical),
    InspectJson("Quarantine", Path.Combine(agentDirectory, "quarantine-status.json"), MissingBehavior.Warning),
    InspectJson("Latest tamper event", Path.Combine(agentDirectory, "tamper-event-latest.json"), MissingBehavior.Healthy,
        "Brak zdarzen tamper - plik nie byl potrzebny."),
    InspectText("Agent events", Path.Combine(agentDirectory, "agent-events.jsonl"), true, 100),
    InspectBinary("Device identity", Path.Combine(agentDirectory, "device-identity.bin"), MissingBehavior.Critical),
    InspectBinary("Policy state", Path.Combine(agentDirectory, "policy-state.bin"), MissingBehavior.Critical),
    InspectBinary("Protected quarantine state", Path.Combine(agentDirectory, "quarantine-state.bin"), MissingBehavior.Healthy,
        "Kwarantanna nigdy nie byla aktywna - chroniony plik nie byl potrzebny."),
    InspectRuntime(),
...
var export = new DiagnosticExport(
    ...
    new DiagnosticSummary(
        modules.Count(m => m.Status == ModuleStatus.Healthy),
        modules.Count(m => m.Status == ModuleStatus.Warning),
        modules.Count(m => m.Status == ModuleStatus.Critical),
        modules.Count),
    modules);
...
static ModuleResult InspectJson(string name, string path, MissingBehavior missingBehavior, string? healthyMissingSummary = null)
{
    if (!File.Exists(path)) return Missing(name, path, missingBehavior, healthyMissingSummary);
    try
    {
        ...
        else if (root.TryGetProperty("stateStatus", out var state))
        {
            var value = state.GetString();
            if (string.Equals(value, "OK", StringComparison.OrdinalIgnoreCase))
                summary = "Stan polityki jest prawidlowy.";
            else if (string.Equals(value, "STATE_INITIALIZED", StringComparison.OrdinalIgnoreCase))
                summary = "Stan polityki zostal prawidlowo zainicjalizowany.";
            else
            {
                status = ModuleStatus.Warning;
                summary = $"Stan modulu wymaga uwagi: {value ?? "brak kodu"}.";
            }
        }

        return new ModuleResult(name, status, summary, path,
            JsonSerializer.Serialize(root, new JsonSerializerOptions { WriteIndented = true }), null);
    }
    catch (Exception ex)
    {
        return new ModuleResult(name, ModuleStatus.Critical, "Nie mozna odczytac JSON.", path, null, ex.ToString());
    }
}

static ModuleResult InspectText(string name, string path, bool required, int tailLines)
{
    if (!File.Exists(path)) return Missing(name, path, required ? MissingBehavior.Critical : MissingBehavior.Warning);
    try
    {
        var lines = File.ReadLines(path).TakeLast(tailLines).ToArray();
        return new ModuleResult(name, lines.Length == 0 ? ModuleStatus.Warning : ModuleStatus.Healthy,
            $"Odczytano ostatnie {lines.Length} wpisow.", path,
            lines.Length == 0 ? "Plik jest pusty." : string.Join(Environment.NewLine, lines), null);
    }
    catch (Exception ex)
    {
        return new ModuleResult(name, ModuleStatus.Critical, "Nie mozna odczytac logu.", path, null, ex.ToString());
    }
}

static ModuleResult InspectBinary(string name, string path, MissingBehavior missingBehavior, string? healthyMissingSummary = null)
{
    if (!File.Exists(path)) return Missing(name, path, missingBehavior, healthyMissingSummary);
    try
    {
        var info = new FileInfo(path);
        return new ModuleResult(name, info.Length > 0 ? ModuleStatus.Healthy : ModuleStatus.Critical,
    ...
    {
        return new ModuleResult(name, ModuleStatus.Critical, "Nie mozna sprawdzic pliku.", path, null, ex.ToString());
    }
}
...
static ModuleResult InspectRuntime()
{
    try
    {
        ...
        return new ModuleResult(".NET Runtime", version.Major >= 8 ? ModuleStatus.Healthy : ModuleStatus.Warning,
            $"Runtime {version}", null, details, null);
    }
    catch (Exception ex)
    {
        return new ModuleResult(".NET Runtime", ModuleStatus.Critical, "Brak danych runtime.", null, null, ex.ToString());
    }
}
...
static ModuleResult InspectEnvironment(string agentDirectory)
{
    try
    {
        ...
        return new ModuleResult("System / Environment", status, $"Wolne miejsce: {freeGb:F2} GB", agentDirectory, details, null);
    }
    catch (Exception ex)
    {
        return new ModuleResult("System / Environment", ModuleStatus.Critical, "Nie mozna odczytac danych systemowych.", agentDirectory, null, ex.ToString());
    }
}

static ModuleResult Missing(string name, string path, MissingBehavior behavior, string? healthySummary = null)
{
    return behavior switch
    {
        MissingBehavior.Healthy => new ModuleResult(name, ModuleStatus.Healthy,
            healthySummary ?? "Brak pliku jest prawidlowy dla obecnego stanu.", path, null, null),
        MissingBehavior.Warning => new ModuleResult(name, ModuleStatus.Warning,
            "Plik jeszcze nie zostal utworzony.", path, null, null),
        _ => new ModuleResult(name, ModuleStatus.Critical,
            "Brak wymaganego pliku.", path, null, null)
    };
}
...
enum MissingBehavior { Healthy, Warning, Critical }
