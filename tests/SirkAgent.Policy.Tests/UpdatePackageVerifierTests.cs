...
public sealed class UpdatePackageVerifierTests : IDisposable
{
    ...
    public void VerifyAcceptsSignedPackageAndRejectsTamper()
    {
        ...
        var unsigned = new UpdateManifest(1, "sirk-agent", "SIRK Agent", "0.1.1.10", "win-x64",
            names.Select(name =>
            {
        ...
        Assert.Equal("UPDATE_VERSION_ROLLBACK", verifier.Verify(_root, manifest, "0.1.1.11").Code);
        File.AppendAllText(Path.Combine(_root, names[0]), "tamper");
        Assert.Equal("UPDATE_FILE_SIZE_MISMATCH", verifier.Verify(_root, manifest).Code);
    }
    ...
    {
        Directory.CreateDirectory(_root);
        var manifest = new UpdateManifest(1, "sirk-agent", "SIRK Agent", "0.1.1.10", "win-x64",
            [new UpdateManifestFile("../outside.exe", 1, new string('A', 64))],
