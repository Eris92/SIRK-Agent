...
module.exports.workspace = function workspacePlugin(parent) {
    ...
    obj.server_startup = function () { console.log('[MeshCentral-Workspace] Plugin 0.8.9 loaded'); };
    ...
    obj.onWebUIStartupEnd = function () {
        ...
        var endpoint = function (asset) { var url = new URL('pluginadmin.ashx', window.location.href); url.searchParams.set('pin', 'workspace'); url.searchParams.set('asset', asset); url.searchParams.set('v', '0.8.9-' + Date.now()); return url.href; };
        ...
        window.MeshCentralWorkspace.bootstrapPromise = load('workspace-main-script', endpoint('main.js')).then(function () { return load('workspace-apps-script', endpoint('apps.js')); }).then(function () { return window.MeshCentralWorkspace.initialize(); }).catch(function (error) { window.MeshCentralWorkspace.bootstrapPromise = null; if (window.console) console.error('Workspace bootstrap error', error); });
    };
    ...
    obj.hook_processAgentData = function (command, agent) { pluginModule.captureAgentData(command, agent); virtualMedia.captureAgentData(command, agent); appControl.captureAgentData(command, agent); };
    ...
    obj.handleAdminPostReq = function (req, res, user) {
        ...
        if (asset === 'media-mount') { handlePromise(res, virtualMedia.mount(user, body.id, body.url)); return; }
        if (asset === 'media-unmount') { handlePromise(res, virtualMedia.unmount(user, body.id)); return; }
