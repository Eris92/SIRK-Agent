...
(function () {
    ...
    function render() {
        ...
        root.innerHTML = '<div class="workspace-header"><div><h2>Workspace</h2><p>Host: ' + escapeHtml(plugin.state.nodeId || "-") + '</p></div><button id="workspace-refresh" class="btn btn-primary btn-sm">Odśwież</button></div>' +
            ...
            '<p class="workspace-note">Workspace A i B tworzą rzeczywiste okno testowe na ukrytym desktopie. Jeżeli „Okno testowe” pokazuje Tak, izolowany pulpit potrafi już uruchamiać GUI bez pokazywania go użytkownikowi.</p>';
        ...
        Array.prototype.forEach.call(root.querySelectorAll(".workspace-card"), function (element) {
            ...
            if (stopButton) stopButton.onclick = function () { var slot = plugin.state.slots.find(function (item) { return item.slot === slotId; }); if (slot && slot.id) stop(slot.id); };
        });
    }
    ...
    function start(slot) { post("start", { nodeId: plugin.state.nodeId, slot: slot }).then(function () { loadSlots(); startPolling(); }).catch(function (error) { alert(error.message); loadSlots(); }); }
    function stop(id) { post("stop", { id: id }).then(function () { loadSlots(); startPolling(); }).catch(function (error) { alert(error.message); loadSlots(); }); }
    ...
    plugin.openDeviceTab = function (event) { if (event && ((event.which === 3) || (event.button === 2))) return false; if (typeof window.putstore === "function") window.putstore("_curPluginPage", "workspace-device-page"); if (typeof window.go === "function") window.go(19, event); window.setTimeout(function () { var header = document.getElementById("p19ph-workspace-device-page"); if (header && window.pluginHandler && typeof window.pluginHandler.callPluginPage === "function") window.pluginHandler.callPluginPage("workspace-device-page", header); loadSlots(); }, 0); if (event && event.preventDefault) event.preventDefault(); return false; };
