...
module.exports.workspace = function workspacePlugin(parent) {
    ...
    function send(res, code, type, body) { res.statusCode = code; res.setHeader('Content-Type', type); res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0'); res.setHeader('Pragma', 'no-cache'); res.end(body); }
    ...
    obj.server_startup = function () { console.log('[MeshCentral-Workspace] Plugin 0.9.10 loaded (resilient DXGI helper download)'); };
    ...
    obj.onWebUIStartupEnd = function () {
        ...
        var endpoint = function (asset) { var url = new URL('pluginadmin.ashx', window.location.href); url.searchParams.set('pin', 'workspace'); url.searchParams.set('asset', asset); url.searchParams.set('v', '0.9.10-' + Date.now()); return url.href; };
    ...
    obj.handleAdminReq = function (req, res, user) {
        ...
        if (asset === 'slots') { sendJson(res, 200, { ok: true, result: pluginModule.list(String(req && req.query && req.query.nodeId || '')) }); return; }
        if (asset === 'capture-image') {
            try { send(res, 200, 'image/png', captureControl.getImage(user, req && req.query && req.query.id)); }
            catch (error) { sendJson(res, 404, { ok: false, error: String(error && error.message || error) }); }
            return;
        }
