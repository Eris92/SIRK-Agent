...
if ($PSCmdlet.ShouldProcess($InstallDirectory, 'Install SIRK Agent')) {
    ...
    # Use well-known SIDs instead of localized account names. This works on
    # Polish, English and other Windows language versions.
    ...
    # Framework-dependent publication requires the EXE, DLL, deps.json,
    # runtimeconfig.json and package assemblies. Copy the complete publish
    # directories, not only the executable files.
