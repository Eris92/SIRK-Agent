...
public static class SirKWorkspaceDesktop091 {
  ...
  public static int Launch(uint sessionId,string desktopName,string file,string arguments) {
    ...
    function launchCommand(session, file, args) {
        ...
        return ["$ErrorActionPreference='Stop'", 'try{', prelude(), "$requested='" + safeFile + "'", "$resolved=if([IO.Path]::IsPathRooted($requested)){[Environment]::ExpandEnvironmentVariables($requested)}else{(Get-Command $requested -CommandType Application -ErrorAction Stop|Select-Object -First 1 -ExpandProperty Source)}", "if(-not(Test-Path -LiteralPath $resolved)){throw ('Application not found: '+$resolved)}", "$launchedPid=[SirKWorkspaceDesktop091]::Launch([uint32]" + Number(session.windowsSessionId || 0) + ",'" + desktop + "',$resolved,'" + safeArgs + "')", resultLine(session.id, 'launched', "([ordered]@{desktop='" + desktop + "';file=$resolved;pid=$launchedPid})"), '}catch{' + resultLine(session.id, 'error', "([ordered]@{message=$_.Exception.Message})") + '}'].join(';');
    }
